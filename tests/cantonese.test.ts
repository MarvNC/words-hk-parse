...
import { parseCantoneseReadings } from '../src/utils/cantonese.js';
import type { TextReadingPair } from '../src/types.js';

interface TestCase {
  text: string;
  reading: string;
  expected?: TextReadingPair[];
  shouldThrow?: boolean;
}
...
for (const { text, reading, expected, shouldThrow } of testCases) {
  ...
  }parseCantoneseReadings: ${text} ${reading}`, () => {
    if (shouldThrow) {
